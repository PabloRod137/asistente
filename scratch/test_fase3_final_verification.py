...
def run_verification():
    ...
    import llm
    orig_generate = llm.generate_response
    captured_system_content = []
    
    def spy_generate_response(message, history, phone_number="web_user"):
        # Llamada normal
        return orig_generate(message, history, phone_number)

    res5 = simular_mensaje(test_user_phone, "Hola, quería saber qué documentos necesito")
    logger.info(f"Respuesta a cliente activo: {res5[:100]}...")
    # Verificar que el contexto inyectado en llm incluye a Ana Lopez
